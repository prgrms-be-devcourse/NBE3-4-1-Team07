...
const getOrderList = async () => {
  const res = await fetch("http://localhost:8080/admin/orderList", {
    method: "GET",
    credentials: "include",
  });
...
export default function admin() {
  ...
  const [orderDetail, setOrderDetail] = useState<OrderDetailResponseDto | null>(
    null
  );
    ...
    <div className="flex h-screen">
      <Sidebar backgroundColor="#FBFBFB" className="min-h-full">
        <Menu>
          ...
          <MenuItem onClick={() => handleMenuClick("productList")}>
            등록상품조회
          </MenuItem>
        </Menu>
      </Sidebar>
